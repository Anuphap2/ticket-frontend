            ...
            <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl border border-zinc-100 text-center">
                ...
                {status === 'loading' && (
                    <div className="space-y-4 py-8">
                        ...
                        <h2 className="text-xl font-semibold text-zinc-900">กำลังยืนยันยอดชำระ...</h2>
                        <p className="text-sm text-zinc-500">กรุณาอย่าเพิ่งปิดหน้าต่างนี้</p>
                    </div>
                )}
                ...
                {status === 'success' && (
                    <div className="animate-in fade-in zoom-in duration-500 space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-zinc-600">ระบบได้รับเงินและยืนยันตั๋วให้พู่กันเรียบร้อย</p>
                        </div>
                        ...
                        <div className="pt-4 space-y-3">
                            ...
                            >
                                <Ticket className="h-5 w-5" /> ดูตั๋วของฉัน <ArrowRight className="h-5 w-5" />
                            </button>
                            <button
                                onClick={() => router.push('/')}
                        ...
                        <p className="text-xs text-zinc-400">ระบบจะพาคุณไปหน้าตั๋วอัตโนมัติในครู่เดียว...</p>
                    </div>
                )}

                {status === 'error' && (
                    <div className="space-y-4 py-8 text-red-600">
                        <div className="mx-auto h-16 w-16 bg-red-50 rounded-full flex items-center justify-center font-bold text-2xl">!</div>
                        <h2 className="text-xl font-bold">เกิดข้อผิดพลาด</h2>
                        <p className="text-sm text-zinc-500">แต่ไม่ต้องกังวล หากคุณจ่ายเงินแล้ว ระบบจะใช้เวลาตรวจสอบครู่หนึ่ง</p>
                        <button
                            onClick={() => router.push('/my-bookings')}
