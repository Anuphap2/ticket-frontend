...
export default function ProfilePage() {
  ...
  // 1. สร้าง Ref สำหรับจับ Element ที่จะทำแอนิเมชัน
  ...
  useEffect(() => {
    if (!loading && user && profileCardRef.current) {
      gsap.fromTo(
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] py-16 px-6 antialiased">
      <div className="max-w-2xl mx-auto">
        ...
        {/* 3. ผูก Ref และใส่ opacity-0 ซ่อนไว้ก่อนให้ GSAP ดึงขึ้นมา */}
        <div
          ref={profileCardRef}
          className="opacity-0 bg-white rounded-[3rem] shadow-[0_30px_100px_rgba(0,0,0,0.04)] border border-zinc-100 overflow-hidden relative"
        >
          {/* Header Banner */}
            ...
            <div className="relative z-10 flex items-center gap-6">
              ...
              <div>
                <h2 className="text-white font-black text-2xl tracking-tighter uppercase italic leading-none">
                  {user.firstName}{" "}
                  <span className="text-indigo-400">{user.lastName}</span>
                </h2>
          ...
          <div className="p-10 md:p-14">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
              <div className="space-y-1">
                <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                  <Sparkles size={12} className="text-amber-400" /> Account
                  Status: Active
                </p>
            ...
            <div className="mt-10 grid grid-cols-1 gap-3">
              <Link
                href="/my-bookings"
                className="flex items-center justify-between p-5 rounded-2xl bg-indigo-50/50 border border-indigo-100 hover:bg-indigo-50 transition-colors group"
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-xs font-black uppercase tracking-widest text-indigo-900">
                    View My Ticket Collection
                  </span>
                </div>
                <ArrowLeft
                  size={16}
                  className="rotate-180 text-indigo-400 group-hover:translate-x-1 transition-transform"
                />
              </Link>
            </div>
...
function InfoCard({
  ...
  return (
    <div className="p-6 rounded-4xl bg-white border border-zinc-100 hover:border-indigo-100 hover:shadow-sm transition-all group">
      <div className="flex items-center gap-4">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#fafafa] px-6">
      <div className="max-w-md w-full bg-white p-10 rounded-[2.5rem] shadow-xl border border-zinc-100 text-center">
        <h2 className="text-xl font-black uppercase italic tracking-tight mb-2 text-zinc-900">
          Access Denied
        </h2>
        <p className="text-zinc-500 text-sm mb-8 font-medium">{message}</p>
        <Link
          href="/"
          className="inline-block px-8 py-4 bg-zinc-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em]"
        >
          Return to Home
        </Link>
      </div>
